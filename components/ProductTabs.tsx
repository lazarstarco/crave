...
const ProductTabs = ({ product }: { product: Product }) => {
  ...
  const fetchParticipants = async (id: string) => {
    try {
      const response = await fetch(
        `http://ec2-3-66-85-50.eu-central-1.compute.amazonaws.com:3001/api/participants?productId=${id}`,
      );
  ...
  const handleVote = async (participantId: string) => {
    try {
      const response = await fetch("http://ec2-3-66-85-50.eu-central-1.compute.amazonaws.com:3001/api/participants", {
        method: "POST",
