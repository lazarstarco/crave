...
const AdminSingleOrder = () => {
  ...
  useEffect(() => {
    const fetchOrderData = async () => {
      const response = await fetch(
        `http://ec2-3-79-230-202.eu-central-1.compute.amazonaws.com:3001/api/orders/${params?.id}`,
      );
    ...
    const fetchOrderProducts = async () => {
      const response = await fetch(
        `http://ec2-3-79-230-202.eu-central-1.compute.amazonaws.com:3001/api/order-product/${params?.id}`,
      );
  ...
  const updateOrder = async () => {
    if (
      ...
      fetch(`http://ec2-3-79-230-202.eu-central-1.compute.amazonaws.com:3001/api/orders/${order?.id}`, {
        method: "PUT",
        headers: {
  ...
  const deleteOrder = async () => {
    ...
    fetch(
      `http://ec2-3-79-230-202.eu-central-1.compute.amazonaws.com:3001/api/order-product/${order?.id}`,
      requestOptions,
    ).then((response) => {
      fetch(
        `http://ec2-3-79-230-202.eu-central-1.compute.amazonaws.com:3001/api/orders/${order?.id}`,
        requestOptions,
      ).then((response) => {
      ...
      <div className="flex flex-col gap-y-7 xl:ml-5 w-full max-xl:px-5">
        ...
        <div>
          ...
          <div className="flex flex-col gap-y-2 mt-10">
            <p className="text-2xl">Subtotal: {order?.total} RSD</p>
            <p className="text-2xl">Tax 20%: {order?.total / 5} RSD</p>
            <p className="text-2xl">Shipping: 350 RSD</p>
            <p className="text-3xl font-semibold">
              Total: ${order?.total + order?.total / 5 + 350}
            </p>
          </div>
          <div className="flex gap-x-2 max-sm:flex-col mt-5">
            <button
              type="button"
              className="uppercase bg-slate-950 px-10 py-5 text-lg border border-black border-gray-300 font-bold text-white shadow-sm hover:bg-blue-600 hover:text-white focus:outline-none focus:ring-2"
              onClick={updateOrder}
            >
              Update order
            </button>
